...
regularOperation = authority_table.regularOperations
dangerousOperation = authority_table.dangerousOperations
...
def is_file_regex(regex):
    # 检查是否包含文件扩展名的模式
    if re.search(r'\.[a-zA-Z0-9]+', regex):
        return True
    return False


def is_directory_regex(regex):
    # 检查是否包含路径分隔符且不包含文件扩展名
    if is_path_regex(regex) and not is_file_regex(regex):
        return True
    return False
...
def reset_globals():
    ...
    previous_path = None  # 重置全局变量


def extract_parameters_from_tree(node, table):
    if node.is_utility() and node.children:
        utility = node.value.strip('"')
        if utility in regularOperation:
            utility_info = regularOperation.get(utility)
            for child in node.children:
                process_node(child, table, utility, utility_info, is_dangerous=False)
        if utility in dangerousOperation:
            utility_info = dangerousOperation.get(utility)
            for child in node.children:
                process_node(child, table, utility, utility_info, is_dangerous=True)
    for child in node.children:
        extract_parameters_from_tree(child, table)
    return table
...
def list_files_with_regex(path, pattern):
    # 将通配符模式转换为正则表达式
    regex_pattern = pattern.replace('.', r'\.').replace('*', '.*') + '$'

    if not regex_pattern:
        print(f"Pattern is empty. No files to match.")
        return []
    ...
    if not output:
        print(f"No files found or error occurred while listing files in '{path}'")
        return []
    ...
    matched_files = [line for line in output.splitlines() if regex.search(line)]
...
def process_node(node, table, utility, utility_info, is_dangerous):
    global previous_path
    if node.is_argument() and node.arg_type not in ["Quantity", "Size", "Program", "Type", "Permission", "Number",
                                                    "ReservedWord", "Timespan"]:
        if utility == "find":
            if node.arg_type == "Path":
                if previous_path is None:
                    previous_path = ""
                previous_path = os.path.join(previous_path, node.value)  # 拼接路径

        if node.arg_type == "Regex":
            if is_file_regex(node.value) or is_path_regex(node.value) or is_directory_regex(node.value):
                base_path = previous_path if previous_path != None else '.'

                files = list_files_with_regex(base_path, node.value.strip('"'))
                if files != []:
                    for file in files:
                        if utility != "chmod" and utility != "chgrp":
                            table.add_entry(
                                os.path.join(previous_path, file),
                                is_dangerous=is_dangerous,
                                superuser=utility_info.get("superuser", False),
                                read=utility_info.get("read", False),
                                write=utility_info.get("write", False),
                                execute=utility_info.get("execute", False),
                                own=False
                            )
                        else:
                            table.add_entry(
                    ...
                    if utility != "chmod" and utility != "chgrp":
                        current = run_command("pwd")
                        path = os.path.abspath(current.strip())
                        if path in table.table:
                            entry = table.get_entry(path)
                            if utility_info.get("superuser", False) and not entry.get('superuser', False):
                                entry['superuser'] = True
                            if utility_info.get("read", False) and not entry.get('read', False):
                                entry['read'] = True
                            if utility_info.get("write", False) and not entry.get('write', False):
                                entry['write'] = True
                            if utility_info.get("execute", False) and not entry.get('execute', False):
                                entry['execute'] = True
                        else:
                            table.add_entry(
                                path,
            ...
            if node.value == ".":
                ...
                if node.value == "{}":
                    for entry in table.table.values():
                        ...
                        if utility_info.get("execute", False) and not entry.get('execute', False):
                            entry['execute'] = True
                        if utility == "chmod" or utility == "chgrp":
                            entry['own'] = True
                else:
                    if previous_path:
                        if previous_path == node.value.strip('"'):
                            full_path = os.path.abspath(previous_path)
                        else:
                            full_path = os.path.join(previous_path, node.value.strip('"'))
                    else:
                        full_path = os.path.abspath(node.value.strip('"'))
    ...
    # 递归处理子节点
    for child in node.children:
        process_node(child, table, utility, utility_info, is_dangerous)
...
def split_string(input_string):
    ...
    # 使用 re.split() 根据 $() 分割字符串，同时保留括号内的内容
...
def parse_command(command):
    ...
    return matches
def process_command(command, table):
    # 递归解析和处理命令中的复合指令
    if '$(' in command:
        # 提取子命令
        sub_commands = re.findall(r'\$\(([^)]+)\)', command)
        for sub_cmd in sub_commands:
            # 递归处理子命令
            process_command(sub_cmd, table)

    # 解析子命令部分
    ast = bash_parser(command)
    pretty_print(ast)
    reset_globals()
    extract_parameters_from_tree(ast, table)
if __name__ == "__main__":

    cmd = input("请输入命令： ")

    # 使用模式匹配解析输入的命令
    parsed_cmds = parse_command(cmd)

    # 创建权限表
    table = authority_table.AuthorityTable()

    # 遍历每个部分并进行解析处理
    for part in parsed_cmds:
        part = part.strip()

        # 只处理非空部分
        if part:
            # 处理每个子命令或命令片段，递归解析复合指令
            process_command(part, table)
